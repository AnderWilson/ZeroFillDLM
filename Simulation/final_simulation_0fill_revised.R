...
# from the folder "ZeroFillDLM/Simulation"
...
# load exposure data
exp_dta <- read_csv("~/ZeroFillDLM/Data/ExposureDataPM25.csv")
...
for (min_gest_age in c(30, 37)) {
  ...
  for (scenario in c("A", "B", "C")) {
    ...
    print("------------------------")
    print("------------------------")
    # A: gest. age indep. of exposure, gest. age effects BW. This is the only scenario used in the manuscript.
    # B: gest. age effects exposure, gest. age effects BW (gest age is mediator).
    # C: scenario A plus adjusted confounding (same as D with confounders included in estimation model). (not used in final manuscript)
    ...
    for (GAcontrol in c(TRUE, FALSE)) {
      ...
      for (sim in 1:nsims) {
        ...
        if (scenario %in% c("A", "C")) {
          # gestational age independent of exposure
          gestage <- sample(
            min_gest_age:42,
            size = n,
            prob = c(gestdist - c(0, gestdist[-length(gestdist)]))[as.character(
              min_gest_age:42
            )],
            replace = TRUE
        ...
        # applied to all scenarios in the final simulation
        if (scenario %in% c("A", "B", "C")) {
      ...
      # load(paste0(
      #   "~/ZeroFillDLM/Simulation/Output/sim_results_revised_scenario_",
      #   scenario,
      #   "_GAcontrol",
      #   GAcontrol,
      #   ".rda"
      ...
      # make table summarizing both cumulative and lag-specific estimates
      final_table <- CE_table |>
        mutate(
          CE_bias = CE_bias,
          CE_RMSE = CE_RMSE,
